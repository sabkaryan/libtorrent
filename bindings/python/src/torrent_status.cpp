...
{
	scope status =
		class_<torrent_status>("torrent_status")
			.def(self == self)
...
#if TORRENT_ABI_VERSION == 1
			.add_property(
				"announce_interval", make_getter(&torrent_status::announce_interval, by_value())
			)
#endif
			...
			.def_readonly("upload_limit", &torrent_status::upload_limit)
			.def_readonly("download_limit", &torrent_status::download_limit)
			...
			.add_property(
				"queue_position", make_getter(&torrent_status::queue_position, by_value())
			)
			...
			.add_property(
				...
				make_getter(&torrent_status::need_save_resume_data, by_value())
			)
#if TORRENT_ABI_VERSION == 1
			...
			.add_property(
				"active_duration", make_getter(&torrent_status::active_duration, by_value())
			)
			.add_property(
				"finished_duration", make_getter(&torrent_status::finished_duration, by_value())
			)
			.add_property(
				"seeding_duration", make_getter(&torrent_status::seeding_duration, by_value())
			)
